/* eslint-disable */
import React, { useState, useContext, lazy } from 'react';
import { Box, Button as MuiButton } from '@mui/material';

import { ObjectContext } from '../../context';
import { useParams } from 'react-router-dom';

// import ClaimsListComponent from "./components/claims/ClaimsList";
// import ClaimCreateComponent from "./components/claims/ClaimsCreate";
// import ClaimDetailComponent from "./components/claims/ClaimsDetail";

const ClaimsListComponent = lazy(
  () =>
    import(
      /* webpackChunkName: "claims" */
      './components/claims/ClaimsList'
    ),
);

const ClaimCreateComponent = lazy(
  () =>
    import(
      /* webpackChunkName: "claims" */
      './components/claims/ClaimsCreate'
    ),
);

const ClaimDetailComponent = lazy(
  () =>
    import(
      /* webpackChunkName: "claims" */
      './components/claims/ClaimsDetail'
    ),
);

const searchfacility = {};

const ClaimsModule = ({ beneficiary, corporate }) => {
  const { state, setState } = useContext(ObjectContext);
  const [view, setView] = useState('list');

  const { client_id } = useParams();

  const handleGoBack = () => {
    setView('list');
    setState((prev) => ({
      ...prev,
      ClientModule: {
        ...prev.ClientModule,
        selectedClient: {},
      },
    }));
  };

  const id = beneficiary ? beneficiary._id : client_id;

  return (
    <Box>
      {view === 'list' && (
        <Box>
          <ClaimsListComponent
            showCreate={() => setView('create')}
            showDetail={() => setView('detail')}
            corporate={corporate}
            beneficiary={beneficiary}
            client_id={id}
          />
        </Box>
      )}

      {view === 'create' && (
        <Box>
          <ClaimCreateComponent
            handleGoBack={handleGoBack}
            showCreate={() => setView('create')}
            showDetail={() => setView('detail')}
            beneficiary={beneficiary}
            client_id={id}
          />
        </Box>
      )}

      {view === 'detail' && (
        <Box>
          <ClaimDetailComponent
            handleGoBack={handleGoBack}
            showCreate={() => setView('create')}
            showDetail={() => setView('detail')}
            beneficiary={beneficiary}
            client_id={id}
          />
        </Box>
      )}
    </Box>
  );
};

export default ClaimsModule;
