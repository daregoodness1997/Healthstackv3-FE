...
const InvoiceDetail = ({ handleGoBack }) => {
  ...
  useEffect(() => {
    ...
    returnStatusMessage();
    if (invoice) {
      setValue('startdate', invoice.startdate ? dayjs(invoice.startdate).format('YYYY-MM-DD') : '');
      setValue('enddate', invoice.enddate ? dayjs(invoice.enddate).format('YYYY-MM-DD') : '');
      setValue('duedate', invoice.duedate ? dayjs(invoice.duedate).format('YYYY-MM-DD') : '');
  }
  }, [state.InvoiceModule]);
  ...
  const handleRenewal = () => {
    /// move to backend
    dealServer
      .patch(state.InvoiceModule.selectedInvoice._id, {
          ...
          >
            ...
            {(user?.currentEmployee?.roles?.includes('Admin') ||
              ...
              <>
                ...
                {invoiceStatus.toLowerCase() == 'approved' && (
                  <GlobalCustomButton color="primary" onClick={handleRenewal}>
                    <ApprovalIcon
                      fontSize="small"
                      sx={{ marginRight: '5px' }}
                    />
