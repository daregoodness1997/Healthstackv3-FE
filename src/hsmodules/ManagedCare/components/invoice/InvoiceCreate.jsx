...
const InvoiceCreate = ({ closeModal, handleGoBack, policies }) => {
  ...
  const createInvoice = async (data) => {
    ...
    await InvoiceServer.create(document)
      .then(async (res) => {
  ...
  const processPlans = () => {
    ...
    policies.forEach((el) => {
      ...
      if (planObject.hasOwnProperty(planId)) {
        ...
        } else planObject[planId].individualplan.push(el.plan);
      } else {
    ...
    keys.forEach((property) => {
      ...
      if (planObject[property].familyplan.length > 0) {
        const familypremium = planObject[property].plan.premiums.find(
          (el) => el.planType === 'Family',
        );
        const amount =
          +familypremium.premiumAmount *
          +planObject[property].familyplan.length;
      ...
      if (planObject[property].individualplan.length > 0) {
        const individualpremium = planObject[property].plan.premiums.find(
          (el) => el.planType === 'Individual',
        );
        const amount =
          +individualpremium.premiumAmount *
          +planObject[property].individualplan.length;
        ...
        <Grid container spacing={2} p={2}>
          ...
          <Grid item lg={12} md={12} sm={12}>
            ...
            <Grid container spacing={1} mb={1.5}>
              ...
              <Grid item lg={2} md={3} sm={4}>
                <Input label="VAT %" disabled={true} register={register("vat")} type="number" />
              </Grid>
                <Input
                  label="Subscribtion Category"
