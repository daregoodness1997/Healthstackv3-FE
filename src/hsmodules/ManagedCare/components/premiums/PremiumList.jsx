...
const FilterMenu = React.lazy(
  () => import("../../../../components/utilities/FilterMenu")
);
...
const GlobalCustomButton = lazy(
  () => import("../../../../components/buttons/CustomButton")
);

const PaymentCreatePage = lazy(() => import("./PaymentCreatePage"));
...
export function BillingList({ openModal, showCreateScreen }) {
  ...
  const getFacilities = async () => {
    ...
    const findProductEntry = await PremServ.find({
      query: {
        "participantInfo.billingFacilityId":
          user.currentEmployee.facilityDetail._id,
        status: {
          $ne: "Fully Paid",
        },
  ...
  const financePlaymentListSchema = [
    ...
    },
    {
      ...
      selector: (row) =>
        row.serviceInfo.duedate
          ? format(new Date(row?.serviceInfo?.duedate), "dd-MM-yy")
          : "",
      sortable: true,
      required: true,
      inputType: "TEXT",
