...
const ClaimsListComponent = ({
  ...
  const handleProvider = async () => {
    ...
    if (provider.current) {
      ...
      //  console.log(provRef)
      const uniqueArr = [
        ...new Set(provRef.current.map((data) => data.provider._id)),
      ];
            ...
            {isHMO && (
              <>
                ...
                >
                  ...
                  {selectedFacility?.claims.length > 0 && (
                    <GlobalCustomButton
                      style={{ margin: '0 10px', fontSize: '0.95rem' }}
                      onClick={handleSectAll}
                    >
