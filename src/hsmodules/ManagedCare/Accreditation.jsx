...
export function AccreditationList({
  ...
  console.log("GetFacilities", accreditation);
  ...
  const providerSchema = [
    ...
    },
    {
      ...
      selector: (row) => row?.details[0].assessmentDetails.summeryDate,
      // dayjs(row?.details[0]?.summertDate).format("DD-MM-YYYY"),
      sortable: true,
      required: true,
      inputType: "DATE",
    },
                ...
                <Grid item xs={12} sm={4} md={6}>
                  <CustomSelect
                    ...
                    options={[
                      ...
                      { value: "Deny", label: "Deny Outrightly" },
                  ...
                  <CustomSelect
                    ...
                    name="type"
                    options={["Approve", "Deny", "Probation"]}
                    register={register("recommendationStatus", {
                      required: true,
                    })}
                  />
                </Grid>
...
export function NewOrganizationView({ showModal, setShowModal, selectedAccr }) {
  ...
  const [conpleted, setCompleted] = useState(false);
  const [recommendationStatus, setRecommendationStatus] = useState(
    facility?.details[0]?.assessmentDetails.recommendation,
  );
  //const history = useHistory()
  ...
  useEffect(() => {
    ...
    setRecommendationStatus(assessment.recommendation);
    const initFormValue = {
      nameofmd: personal?.mdName,
      mcdnNo: personal?.Mcdn,
      mdPhone: personal?.mdPhone,
  ...
  const handleClick = async (data) => {
    console.log(
      "accre details before",
      //   dayjs(data.summerydate).format("DD-MM-YYYY"),
      //   data.recommendation
      facility,
    );
    setLoading(true);
    ...
    const accreditationDetails = {
      ...
      assessmentDetails: {
        ...
        ReviewOfficer: data.reviewofficername,
        ReviewDate: data.reviewdate,
        approve: data.recommendation,
        // deny: data.deny,
        // probation: data.probation,
        summeryDate: dayjs(data.summerydate).format("DD-MM-YYYY"),
        completed: data.conpleted,
        recommendation: recommendationStatus, //data.recommendation,
      },
    };
    const allData = {
      organizationName: facility?.organizationDetail?.facilityName,
    ...
    // .create(allData)
    await accreditationServ
      .patch(facilityID, allData)
      .then((resp) => {
        setLoading(false);
        console.log("accrd after ", resp);
        setShowModal(0);
        setState((prev) => ({
          ...prev,
          facilityModule: {
