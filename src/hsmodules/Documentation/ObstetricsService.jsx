...
const ObstetricsServices = () => {
    ...
    const onSubmit = async data => {
        ...
        document.documentname = 'Anc Booking Form'
        // document.documentType = "Obstetrics Services";
        document.location =
        ...
        if (!!draftDoc && draftDoc.status === 'Draft') {
            await ClientServ.patch(draftDoc._id, document)
                .then(res => {
                    toast.success(
                        'Obstetrics Services Document succesfully updated',
                    )
                    reset(data)
                })
                .catch(err => {
            ...
            await ClientServ.create(document)
                .then(res => {
                    toast.success('Obstetrics Services created succesfully')
                    reset(data)
                    closeForm()
                })
                .catch(err => {
                    toast.error('Error creating Obstetrics Services ' + err)
                })
            hideActionLoader()
        }
        //}
    ...
    return (
        <Box sx={{ width: '100%' }}>
            ...
            >
                <FormsHeaderText text="Obstetrics Services" />
...
export default ObstetricsServices
