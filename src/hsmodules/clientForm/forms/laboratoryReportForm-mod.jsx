...
export default function LaboratoryReportForm() {
  ...
  useEffect(() => {
    if (order.resultDetail?.labFormType == null) {
      ...
      setState((prevstate) => ({
        ...prevstate,
        labFormType: state.financeModule.selectedFinance.resultDetail.labFormType,
      }));
    }
...
export function Haematology() {
  ...
  const {
    ...
    defaultValues: {
      labResults: state.labFormType.tests.map(test => ({
        test: test.test,
        value: '',
        unit: test.normalValues[0]?.unitMeasure || '',
        min: test.normalValues[0]?.lowerLimit || '',
        max: test.normalValues[0]?.upperLimit || '',
        status: 'No Input'
      }))
    }
  });
  ...
  useEffect(() => {
    if (!order.resultDetail?.documentdetail) {
      ...
      setValue("Recommendation", "", { shouldValidate: true, shouldDirty: true });
      return;
    }
    if (order.report_status !== "Pending") {
      Object.entries(order.resultDetail.documentdetail).forEach(
        ([keys, value]) => setValue(keys, value, { shouldValidate: true, shouldDirty: true })
      );
    }
  }, [order, setValue]);
  ...
  const updateStatus = useCallback((index) => {
    const { value, min, max } = getValues(`labResults[${index}]`);
  ...
  const getNormalRange = useCallback((test, selectedUnit) => {
    const selectedUnitData = test.normalValues.find((unitData) => unitData.unitMeasure === selectedUnit);
    return selectedUnitData || {};
  }, []);

  const handleSelectChange = useCallback((e, index, test) => {
    const selectedUnit = e.target.value;
    const { lowerLimit: min, upperLimit: max } = getNormalRange(test, selectedUnit);
    console.log(min, max)
  ...
  const recommendationText = useMemo(() => {
    ...
    return labResults.map((lab, index) => (
      <React.Fragment key={index}>
        {state.labFormType?.tests[index]?.test} : {lab.value} ({lab.status}) {lab.unit} {lab.min} {lab.max}
  ...
  {!!state.labFormType.tests && state.labFormType.tests.map((data, index) => (
    <Grid key={index} item xs={6} mb={2}>
      ...
      <Grid item xs={6}>
              ...
              <Grid item xs={6}>
                <CustomSelect
                  ...
                  options={data.normalValues.map((unit) => unit.unitMeasure)}
        ...
        <Grid item xs={12} sm={6}>
          ...
          >
            {labResults[index]?.status} ({labResults[index]?.min} - {labResults[index]?.max})
          </Typography>
        </Grid>
      </Grid>
    </Grid>
