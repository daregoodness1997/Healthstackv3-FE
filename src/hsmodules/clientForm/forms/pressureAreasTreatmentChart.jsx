  ...
  const handleSave = () => {
    ...
    ClientServ.create(document)
      .then((res) => {
        ...
        toast.success("Pressure Areas Treatment sheet entry successful");
      })
      .catch((err) => {
    ...
    if (chosen1) {
      ...
      ClientServ.create(document)
        .then((res) => {
          ...
          reset(data);
          setValue("repositioningDateAndTime", null);
        ...
        .catch((err) => {
          hideActionLoader();
          setConfirmDialog(false);
          toast.error("Pressure Areas Treatment Sheet " + err);
        });
    } else {
      ClientServ.patch(fac.current._id, {
        ...
        .catch((err) => {
          setConfirmDialog(false);
          hideActionLoader();
          toast.error("Pressure Areas Treatment Sheet entry " + err);
        });
    }
  };

  const PressureAreasTreatmentSchema = [
    {
      name: "S/N",
      key: "sn",
      description: "SN",
    ...
    {
      name: "Date & Time",
      key: "repositioningDateAndTime",
      description: "Date & Time",
      selector: (row) =>
        dayjs(row.repositioningDateAndTime).format("DD-MM HH:mm:ss"),
      sortable: true,
      required: true,
      inputType: "TEXT",
    },
    ...
    {
      name: "Repositioning (using Codes)",
      key: "repositioningCodes",
      description: "Repositioning (using Codes)",
      selector: (row) => `${row.repositioningFrom} to ${row.repositioningTo}`,
      sortable: true,
      required: true,
      inputType: "TEXT",
    },
    ...
    {
      name: "Pressure Area Treatment",
      key: "pressureAreaTxt",
      description: "Pressure Area Treatment",
      selector: (row) => row.pressureAreaTxt,
      sortable: true,
      required: true,
      inputType: "TEXT",
    },
    ...
    {
      name: "Skin Inspection Comments",
      key: "skinInspectionComments",
      description: "Skin Inspection Comments",
      selector: (row) => row.skinInspectionComments,
      sortable: true,
      required: true,
      inputType: "TEXT",
    },

    {
      ...
      selector: (row) => row.nameOrSignature,
      sortable: true,
      required: true,
      inputType: "TEXT",
    },
  ...
  return (
    <div className="card">
      <CustomConfirmationDialog
        ...
        type="create"
        message={`You are about to create a Pressure Areas Treatment Chart ?`}
      />
      <Box
        sx={{
          display: "flex",
      ...
      <form>
        ...
        <Box mb="1rem">
          <GlobalCustomButton
            ...
            text={`Submit Pressure Area Treatment Chart`}
          />
        </Box>
      </form>
      <Box>
        <CustomTable
          title={"Submit Pressure Area Treatment Sheet"}
          columns={PressureAreasTreatmentSchema}
          data={facilities}
