  ...
  const getFacilities = async () => {
    const findTransfer = await TransferEntryServ.find({
      query: {
        ...
        $select: ['org_date', 'dest_store_name', 'productitems', 'dest_status','dest_trx_status'],
...
export function TransferReceiveList({ openRecvDetailModal, toggleView }) {
  ...
  const getFacilities = async () => {
    console.log(state.InventoryModule.selectedInventory._id)
    const findTransfer = await TransferEntryServ.find({
      query: {
        // facility: user.currentEmployee.facilityDetail._id,
        $select: ['org_date', 'org_store_name', 'productitems', 'dest_status', 'dest_trx_status'],
  ...
  const submitTransfer = async (e) => {
    ...
    for (const itemId in itemStates) {
      ...
      (transferEntry.rev_date = new Date()),
        ...
        (transferEntry.source = state.InventoryModule.selectedInventory.name); //dest_store_name
      transferEntry.sourceId = state.InventoryModule.selectedInventory._id; // dest_storeId
      transferEntry.sourceType = 'Store';
      ...
      productServ
        .find({
          query: {
            ...
            facility: user.currentEmployee.facilityDetail._id,
            storeId: storeId, //change to source store
            $limit: 10,
            $sort: {
              createdAt: -1,
            },
