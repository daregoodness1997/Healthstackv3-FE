...
export default function ProductEntry() {
  ...
  //   setProductList((prev) => "spending")
  // }
  // const selectProduct = (updates) => {
  ...
  const getFacilities = async () => {
    const findTransfer = await TransferEntryServ.find({
      query: {
        ...
        $select: ['org_date', 'dest_store_name', 'productitems', 'dest_status'],
...
export function TransferReceiveList({ openRecvDetailModal, toggleView }) {
  ...
  const getFacilities = async () => {
    const findTransfer = await TransferEntryServ.find({
      query: {
        // facility: user.currentEmployee.facilityDetail._id,
        $select: ['org_date', 'org_store_name', 'productitems', 'dest_status'],
  ...
  useEffect(() => {
    ...
    getFacilities();
    return () => {};
  }, [state.InventoryModule.selectedInventory._id,limit, page]);
          ...
          >
            <TableMenu>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <h2 style={{ marginLeft: '10px', fontSize: '0.95rem' }}>
                  List of Requisition Inbound
                </h2>
                <div style={{ marginLeft: '10px', fontSize: '0.95rem' }}>
                  <GlobalCustomButton onClick={toggleView}>
                    View requisition Outbound
                  </GlobalCustomButton>
                </div>
              </div>
...
export function TransferDetail({ openModifyModal }) {
  ...
  if (ProductEntry.dest_status === 'Pending') {
    return (
      // <ModalBox header={"hello"}
      // open={true} onClose={handleCloseModal}>
      <EditDetails
...
export function EditDetails({ closeModal, ProductEntry }) {
  ...
  const [source, setSource] = useState('');
  const [date, setDate] = useState('');
  // const [name, setName] = useState(ProductEntry.productitems[0].name);
  // const [baseunit, setBaseunit] = useState(
  //   ProductEntry.productitems[0].baseunit
  // );
  // const [quantity, setQuantity] = useState(
  //   ProductEntry.productitems[0].quantitySent
  // );
  ...
  // const [storeName, setStoreName] = useState(ProductEntry.dest_store_name);
  ...
  const productItemI = ProductEntry.productitems.map((item) => ({
    ...
    //name: item.name,
    // create new key with old value
    //quantitySent: undefined, // remove old key (optional)
  }));
  ...
  // const productItemI = [
  //   {
  //     dest_storeId: ProductEntry.dest_storeId,
  //     org_storeId: ProductEntry.org_storeId,
  //     type: ProductEntry.type,
  //     productitems: ProductEntry.productitems.map((item) => ({
  //       ...item,
  //       quantity: item.quantitySent,
  ...
  // console.log(productItemI, ProductEntry);
 ...
 */
  ...
  //   console.log(row);
  //   const { quantity, name, baseunit, productId } = row;
  //   setName(name);
          ...
          <Grid container spacing={1}>
            <Grid item lg={6} md={6} sm={8} xs={12}>
              <ProductSearch
                ...
                clear={success}
              />
              {/* // value={ProductEntry.productitems[0].name}
              //name="productId" */}
              <input
                className="input is-small"
    ...
    for (const itemId in itemStates) {
      ...
      TransferEntryServ.patch(ProductEntry._id, transferEntry)
        .then(async (res) => {
          ...
          console.log(n);
          //  alert("saved")
          toast.success('Transfer accepted succesfully' + n);
        })
        .catch((err) => {
          // hideActionLoader();
          setConfirmDialog(false);
          toast.error('Error accepting Transfer ' + err);
        });
    }
  };
    ...
    <>
      ...
      >
        <CustomConfirmationDialog
          ...
          cancelAction={() => setConfirmDialog(false)}
          type="create"
          confirmationAction={() => submitTransfer()}
          // message="Are you sure you want to save this transfer ?"
        />
        <Grid container spacing={1} mb={1}>
          <Grid item lg={4} md={6} sm={6} xs={12}>
        ...
        >
          <GlobalCustomButton
            disabled={!ProductEntry.productitems.length > 0}
            onClick={() => setConfirmDialog(true)}
            sx={{
              marginRight: '10px',
            }}
