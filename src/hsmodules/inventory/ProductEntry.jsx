...
// console.log(costprice, "isPureLife")
  ...
  const getSearchfacility = (obj) => {
    setProductId(isPureLife ? obj?.id : obj?.product?._id);
    setName(isPureLife ? obj?.name : obj?.name);
    setBaseunit(obj?.baseunit);
    setCostprice(isPureLife ? obj?.lst_price : '');
  ...
  const onSubmit = async (e) => {
    ...
    // console.log(state.employeeLocation.locationId, "store");
    ...
    let productEntry = {
      ...
      productitems: productItem.map((item) => ({
        ...item,
      })),
    };
    ...
    const notificationObj = {
      ...
      priority: 'normal',
      dest_locationId: [state.InventoryModule.selectedInventory._id],
    };
...
export function ProductEntryList({ openCreateModal, openDetailModal }) {
  ...
  const searchProducts = async (query) => {
    try {
      const res = await ProductEntryServ.find({ query });
  ...
  const fetchProductEntries = async (query) => {
    setLoading(true);
    try {
      const resp = await ProductEntryServ.find({ query });
      setTotal(resp.total);
  ...
  useEffect(() => {
    if (!state.InventoryModule.selectedInventory) {
      toast({
    ...
    const eventHandlers = {
      created: getUpdatedFacilities,
      updated: getUpdatedFacilities,
      patched: getUpdatedFacilities,
      removed: getUpdatedFacilities,
    };

    Object.entries(eventHandlers).forEach(([event, handler]) => {
      ProductEntryServ.on(event, handler);
    });

    return () => {
      Object.entries(eventHandlers).forEach(([event, handler]) => {
        ProductEntryServ.off(event, handler);
      });
    };
  }, []);
  ...
  useEffect(() => {
    getNewFacilities();
  }, [state.employeeLocation.locationId]);

  // console.log(facilities, "facilities");
  // console.log(state.employeeLocation.locationId, "locationId");

  const handleDelete = async (obj) => {
    try {
      await ProductEntryServ.remove(obj._id);
      toast.success('Successfully deleted ProductEntry');
    } catch (err) {
