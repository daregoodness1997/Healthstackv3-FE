...
export function CheckIn({ pageView, setPageView, setShowModal }) {
    ...
    const handleSearch = val => {
        ...
        ClientServ.find({ query: query })
            .then(res => {
                ...
                const checkedIn = res.data.filter(
                    item =>
                        item.appointment_status == 'Checked In' &&
                        item.location_type == 'Radiology',
                )
                setFacilities(checkedIn)
            ...
            .catch(err => {
                console.log(err)
                setMessage(
                    'Error fetching Client, probable network issues ' + err,
                )
                setError(true)
            })
    }
    ...
    const getFacilities = async () => {
        if (user.currentEmployee) {
            ...
            const res = findClient.data.filter(
                item =>
                    item.appointment_status == 'Checked In' &&
                    item.location_type == 'Radiology',
            )

            await setFacilities(res)
        } else {
            if (user.stacker) {
...
export function CheckOut({ pageView, setPageView, showModal, setShowModal }) {
    ...
    const handleSearch = val => {
        ...
        ClientServ.find({ query: query })
            .then(res => {
                console.log(res)
                const checkedOut = res.data.filter(
                    item =>
                        item.appointment_status == 'Checked Out' &&
                        item.location_type == 'Radiology',
                )
                setFacilities(checkedOut)
