...
import AppointmentListRefactored from './refactored/AppointmentListRefactored';
  ...
  return (
    <section className="section remPadTop">
      <AppointmentListRefactored
        ...
        onOpenModify={() => setShowModal(true)}
      />
...
export function AppointmentCreate({ showModal, setShowModal }) {
  ...
  const onSubmit = (data) => {
    ...
    const notificationObj = {
      ...
      description: `You have a schedule appointment with ${chosen.firstname} ${chosen.lastname
    ...
    const emailObj = {
      ...
      html: `<p>You have been scheduled for an appointment with ${chosen2.profession
        } ${chosen2.firstname} ${chosen2.lastname} at ${dayjs(
          data.start_time,
        ).format('DD/MM/YYYY hh:mm')} ${isHMO ? `and your OTP code is ${generatedOTP}` : ''
        } </p>`,

      text: `You have been scheduled for an appointment with ${chosen2.profession
        } ${chosen2.firstname} ${chosen2.lastname} at ${dayjs(
          data.start_time,
        ).format('DD/MM/YYYY hh:mm')} ${isHMO ? `and your OTP code is ${generatedOTP}` : ''
        } `,
      status: 'pending',
      subject: `SCHEDULED APPOINTMENT WITH ${employee.facilityDetail.facilityName
    ...
    const smsObj = {
      message: `You have been scheduled for an appointment with ${chosen2.profession
        } ${chosen2.firstname} ${chosen2.lastname} at ${dayjs(
          data.start_time,
        ).format('DD/MM/YYYY hh:mm')} ${isHMO ? `and your OTP code is ${generatedOTP}` : ''
        } `,
...
export function ClientList({ showModal, setShowModal }) {
  ...
  const mapFacilities = () => {
    ...
    facilities.map((facility, i) => {
      mapped.push({
        title: `Name: ${facility?.firstname} ${facility?.lastname
          }. Age: ${formatDistanceToNowStrict(
            new Date(facility?.dob),
          )}. Gender: ${facility?.gender}. Phone: ${facility?.phone}. Email: ${facility?.email
          }`,
        startDate: format(
