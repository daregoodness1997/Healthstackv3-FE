...
import { getDayIntheOtherMonth } from "../getNameAndNumOfDaysArray";
...
export const PolicyCheckBoxValue = (service) => {
  const { totalPolicy } = TotalPolicy(service);
  const { totalApprovedPolicy } = TotalApprovedPolicy(service);
  const { totalPendingPolicy, isPending } = TotalPendingPolicy(service);

  const policyCheckBoxValue = {
    totalCheckOne: totalPolicy,
    totalCheckTwo: totalApprovedPolicy,
    totalCheckThree: totalPendingPolicy,
  };
  return { policyCheckBoxValue, isPending };
};
