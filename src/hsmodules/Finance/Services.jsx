...
export function ServicesCreate({ closeModal }) {
  ...
  const productItemSchema = [
    ...
    },
    {
      ...
      selector: (row) => (
        <p style={{ color: "red", fontSize: "0.7rem" }}>Remove</p>
      ),
      sortable: true,
      required: true,
      inputType: "TEXT",
...
export function ServicesList({ openCreateModal, openDetallModal }) {
  ...
  const hospitalInfo = user?.currentEmployee?.facilityDetail
  // const [selectedService, setSelectedService] = useState(false)
  // console.log(user.currentEmployee?.facilityDetail, "NAME")
  ...
  // const handleDeleteService = async (category, id) => {
  //     const updatedServices = selectedServices.filter(item => item._id !== id)
  //     const selectedCategory = facilities.filter(
  //         item => item.categoryname == category,
  //     )
  //     selectedCategory.services = updatedServices

  //     console.log(id)
  //     ServicesServ.patch(id, selectedCategory)
  //         .then(() => {
  //             toast.success('Services deleted succesfully')
  //         })
  //         .catch(err => {
  //             toast.error(
  //                 'Error deleting Services, probable network issues or ' +
  //                     err,
  //             )
  //         })
  // }

  const handleGenerateLink = async (serviceId, serviceName, servicePrice) => {
    try {
      const PriceLinkServ = client.service('pricelink');

      // Create price link record
      const payload = {
      ...
      await PriceLinkServ.create(payload);

      // Generate the link - you can customize this URL structure
      const baseUrl = window.location.origin;
      const facilityName = user?.currentEmployee.facilityDetail?.facilityName || "";
      const formattedFacilityName = facilityName.toLowerCase().replace(/\s+/g, '-');
      const serviceLink = `${baseUrl}/services/${formattedFacilityName}/${serviceId}`;
      ...
      toast.success(`Link copied for ${serviceName}!`);
      ...
      toast.error('Failed to generate/copy link: ' + err.message);
    }
  };
  ...
  const handleDelete = async (deleteId) => {
    ...
    try {
      await ServicesServ.remove(deleteId);
      toast.success("Service deleted successfully");
      setSelectedServices([]);
    } catch (err) {
      toast.error("Error deleting service, probable network issues or " + err);
    }
  };
  ...
  const confirmDelete = (deleteId) => {
    setConfirmationDialog({
      open: true,
      confirmAction: () => handleDelete(deleteId),
    });
  };
      ...
      selector: (row, i) => (
        <DeleteOutline
          ...
          onClick={() => {
            ...
            confirmDelete(row.services[0]._id);
          }}
        />
      ),
      sortable: true,
    ...
    },
    {
      ...
      selector: row => (
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <GlobalCustomButton
            onClick={() => {
              const price = row.contracts.find(el => el.source_org === el.dest_org)?.price;
              handleGenerateLink(row._id, row.name, price);
            }}
            sx={{
              mr: 1,
              backgroundColor: copiedLinkId === row._id ? '#4caf50' : '#1976d2',
              '&:hover': {
                backgroundColor: copiedLinkId === row._id ? '#45a049' : '#115293',
              },
            }}
          >
            {/* <LinkIcon fontSize="small" sx={{ mr: 0.5 }} /> */}
            {copiedLinkId === row._id ? "Copied!" : "Generate Link"}
          </GlobalCustomButton>
        </Box>
      ),
      sortable: false,
  ...
  return (
    <>
      <CustomConfirmationDialog
        ...
        cancelAction={() => setConfirmationDialog({ open: false })}
        confirmationAction={confirmationDialog.confirmAction}
        type="delete"
