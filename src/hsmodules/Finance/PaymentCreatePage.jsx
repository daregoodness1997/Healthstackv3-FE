...
export default function PaymentCreatePage({ closeModal, handleGoBack }) {
  ...
  // console.log(medication, "finance");
  ...
  useEffect(() => {
    includesArt
      ? setSource(medication?.participantInfo?.client?.name)
      : setSource(
  ...
  const handleBulkPayment = async () => {
    // 1. Check for partial payment conditions
    if (part) {
      if (!partBulk || partBulk <= 0) {
        toast.error("Please enter an amount as part payment");
        return;
      }
      ...
      // Calculate how much of each bill can be paid with the partial amount
      let remainingPartialAmount = partBulk;

      productItem.forEach((el) => {
        // Calculate the amount to pay for this specific bill
        const billAmount = Math.min(
          el.proposedpayment.amount,
          remainingPartialAmount
        );

        remainingPartialAmount = Math.max(
          0,
          remainingPartialAmount - billAmount
        );
        ...
        // Update proposed payment
        el.proposedpayment = {
          balance: Number(el.paymentInfo.balance) - Number(billAmount),
          paidup: Number(el.paymentInfo.paidup) + Number(billAmount),
          amount: billAmount,
        };
      });
    } else {
    ...
    // Update all items with new payment status
    let allItems = productItem;
    allItems.forEach((el) => {
      el.paymentInfo.balance = el.proposedpayment.balance;
      el.paymentInfo.paidup = el.proposedpayment.paidup;
      ...
      el.billing_status =
        el.paymentInfo.balance === 0 ? "Fully Paid" : "Part Payment";
      if (el.billing_status === "Part Payment") {
        setIsPart(true);
      }
    ...
    const obj = {
      ...
      totalamount: totalamount,
      createdby: user._id,
      status: part ? "Part Payment" : "Fully Paid",
      bills: allItems,
      balance: balance,
      facilityName: user.currentEmployee.facilityDetail.facilityName,
      subwallet: subWallet,
    ...
    try {
      const resp = await InvoiceServ.create(obj);
      toast.success("payment successful");
      printreceipt(obj);
      setPartBulk("");
      setPart(false);
  ...
  const handleUsbPrint = async () => {
    const dataToBePrinted = {
      ...
      address: user.currentEmployee.facilityDetail.facilityAddress,
