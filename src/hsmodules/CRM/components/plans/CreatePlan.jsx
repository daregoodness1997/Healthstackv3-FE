...
export const PageCreatePlan = ({ addNewPlan }) => {
  ...
  const handleOnPlanSelect = (plan) => {
    ...
    setValue("name", `${plan?.planName} (${plan.planCategory})`);
  ...
  const calculatePlanAmount = useCallback(() => {
    //console.log(premium);
    let premiumUnit=1
    if (!premium || !calendrical || !length || !heads||!selectedPlan.planName) return;
    if (selectedPlan.premiumDurationType==="Months"){
      premiumUnit=(Number(selectedPlan.premiumAmount)/Number(selectedPlan.premiumDuration))*12

    }
    if (selectedPlan.premiumDurationType==="Years"){
      premiumUnit=(Number(selectedPlan.premiumAmount)/Number(selectedPlan.premiumDuration))

    }


    if (calendrical === "Year(s)") {
      const amount = Number(premiumUnit) * Number(length);
      //console.log(amount);
      const headsAmount = Number(amount) * Number(heads);
      setValue("amount", Number(headsAmount.toFixed()));
    } else {
      const numOfYears = length;
      const numOfYearsToMonths = Number(length) / 12;

      const amount = Number(premiumUnit) * Number(numOfYearsToMonths);
      const headsAmount = Number(amount) * Number(heads);
      setValue("amount", Number(headsAmount.toFixed()));
    }
          ...
          <Grid item lg={6} md={6} sm={6}>
            <Input
              register={register("amount", {
                required: true,
                setValueAs: (v) => parseFloat(parseFloat(v).toFixed()),
              })}
              label="Amount"
              type="number"
              //placeholder="Enter customer number"
