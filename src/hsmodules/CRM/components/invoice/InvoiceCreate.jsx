...
const InvoiceCreate = ({ closeModal, handleGoBack }) => {
  ...
  const createInvoice = async (data) => {
    ...
    const totalAmount = plans.reduce((accumulator, object) => {
      return Number(accumulator) + Number(object.amount);
    }, 0);

    const vatAmount = (data.vat / 100) * totalAmount;
    const markupAmount = (data.markup / 100) * totalAmount;
    const finalTotalAmount = totalAmount + vatAmount + markupAmount;
    ...
    const document = {
      ...
      status: "Pending",
      _id: uuidv4(),
      total_amount: Number(finalTotalAmount.toFixed()),
    };
    ...
    await dealServer
      ...
      .then(async (res) => {
        // console.log(res, "created invocies");
          ...
          <Grid item lg={12} md={12} sm={12}>
            ...
            <Grid container spacing={1} mb={1.5}>
              ...
              <Grid item lg={2} md={3} sm={4}>
                <Input
                  label="Markup %"
                  register={register("markup")}
                  type="number"
                />
              </Grid>
              <Grid item lg={2} md={3} sm={4}>
                <Input label="VAT %" register={register("vat")} type="number" />
              </Grid>
              <Grid item lg={2} md={3} sm={4}>
                <CustomSelect
                  label="Payment Option"
                  options={["Annually", "Monthly", "Bi-Annually", "Quarterly"]}
                  control={control}
                  name="payment_option"
                  required
                />
...
export const HealthPlanSearchSelect = ({ handleChange, clearValue }) => {
  ...
  const getFacilities = async () => {
    if (user.currentEmployee) {
      ...
      //console.log(findHealthPlan.data)
  ...
  return (
    <Autocomplete
      ...
      autoHighlight
      getOptionLabel={(option) => `${option.planName}-${option.premiumDuration}${option.premiumDurationType} (${option.planType}) `}
      renderOption={(props, option) => (
        <Box component="li" {...props} sx={{ fontSize: "0.85rem" }}>
          {option.planType} - {option.premiumAmount} for {option.premiumDuration}{option.premiumDurationType}
        </Box>
      )}
      renderInput={(params) => (
        <TextField
