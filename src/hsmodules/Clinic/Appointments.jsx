...
export function ClientList() {
  ...
  const handleCreateNew = async () => {
    const newClientModule = { selectedAppointment: {}, show: "create" };
    await setState(prev => ({ ...prev, AppointmentModule: newClientModule }));
    await setState(prev => ({ ...prev, ClientModule: { selectedClient: {}, show: "create" } }));
  };

  const handleRow = async Client => {
    await setSelectedAppointment(Client);
    await setState(prev => ({ ...prev, AppointmentModule: { selectedAppointment: Client, show: "detail", selectedAllAppointment: Client } }));
  };
  ...
  const buildQuery = (searchValue = "") => {
    const baseQuery = {
      $or: [
        ...
        { appointment_type: { $regex: searchValue, $options: "i" } },
        { appointment_status: { $regex: searchValue, $options: "i" } },
        { appointment_reason: { $regex: searchValue, $options: "i" } },
        { location_type: { $regex: searchValue, $options: "i" } },
        { location_name: { $regex: searchValue, $options: "i" } },
        { practitioner_department: { $regex: searchValue, $options: "i" } },
        { practitioner_profession: { $regex: searchValue, $options: "i" } },
        { practitioner_name: { $regex: searchValue, $options: "i" } }
      ],
    ...
    if (viewBranchOnly) {
      baseQuery.facility = user.currentEmployee.facilityDetail._id;
    }

    if (state.employeeLocation.locationType !== "Front Desk") {
      baseQuery.locationId = state.employeeLocation.locationId;
    }
  ...
  const getFacilities = async () => {
    try {
      ...
      if (viewBranchOnly) {
        query.facility = user.currentEmployee.facilityDetail._id;
      }
  ...
  const handleCalendarClose = async () => {
    const query = {
      start_time: { $gt: subDays(startDate, 1), $lt: addDays(startDate, 1) },
      // facility: user.currentEmployee.facilityDetail._id,
      $limit: 100,
      $sort: { createdAt: -1 }
    };
  ...
  useEffect(() => {
    if (user) {
      ...
      return () => {
        ClientServ.off("created", handleCalendarClose);
        ClientServ.off("updated", handleCalendarClose);
        ClientServ.off("patched", handleCalendarClose);
        ClientServ.off("removed", handleCalendarClose);
      };
    }
  }, [user, viewBranchOnly]);
      ...
      {user ? (
        <>
          <div className="level">
            ...
            <div className="level-item">
              <span className="is-size-6 has-text-weight-medium">Appointments</span>
            </div>
            <div className="level-right">
              ...
              <div className="level-item">
                <button className="button is-success is-small" onClick={handleCreateNew}>
                  New
                </button>
              </div>
            </div>
              ...
              <tbody>
                {facilities.map((Client, i) => (
                  <tr
                    ...
                    className={Client._id === (selectedAppointment?._id || null) ? "is-selected" : ""}
                  >
                    <th>{i + 1}</th>
                    <td>
                      <strong>{format(new Date(Client.start_time), "dd-MM-yy HH:mm:ss")}</strong>
                    </td>
