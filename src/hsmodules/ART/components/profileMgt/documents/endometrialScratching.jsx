...
export default function EndometrialScratching() {
  ...
  const onSubmit = async (data) => {
    ...
    let document = {
      documentdetail: data,
      documentname: "Endometrial Scratching",
    ...
    try {
      if (!!draftDoc && draftDoc.status === "Draft") {
        ...
        toast.success("Endometrial scratching updated successfully");
      } else {
        await ARTClinicalDocumentServ.create(document);
        toast.success("Endometrial scratching created successfully");
      }

      closeEncounterRight();
    } catch (err) {
      toast.error("Error submitting Endometrial scratching: " + err);
    } finally {
      hideActionLoader();
    }
  };
    ...
    <>
      <div className="card ">
        <Box
          ...
          mb={1}
        >
          <FormsHeaderText text={"Endometrial Scratching"} />
