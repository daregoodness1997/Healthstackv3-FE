...
export default function ReviewOfResultsCreate() {
  ...
  const onSubmit = async (data) => {
    ...
    let document = {
      documentdetail: data,
      documentname: "Review Of Results",
    ...
    try {
      if (!!draftDoc && draftDoc.status === "Draft") {
        ...
        toast.success("Review Of Results updated successfully");
      } else {
        await ARTClinicalDocumentServ.create(document);
        toast.success("Review Of Results created successfully");
      }

      closeEncounterRight();
    } catch (err) {
      toast.error("Error submitting Review Of Results: " + err);
    } finally {
      hideActionLoader();
    }
  };
    ...
    <>
      <div className="card ">
        <Box
          ...
          mb={1}
        >
          <FormsHeaderText text={"Review of results and ivf discussion"} />
