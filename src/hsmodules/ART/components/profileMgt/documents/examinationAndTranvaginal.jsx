...
export default function ExaminationAndTransvaginalScan() {
  ...
  const onSubmit = async (data) => {
    ...
    let document = {
      documentdetail: data,
      documentname: "Examination And Transvaginal Scan",
    ...
    try {
      if (!!draftDoc && draftDoc.status === "Draft") {
        ...
        toast.success("Examination and transvaginal scan updated successfully");
      } else {
        await ARTClinicalDocumentServ.create(document);
        toast.success("Examination and transvaginal scan created successfully");
      }

      closeEncounterRight();
    } catch (err) {
      toast.error("Error submitting Examination and transvaginal scan: " + err);
    } finally {
      hideActionLoader();
    }
  };
    ...
    <>
      <div className="card ">
        <Box
          ...
          mb={1}
        >
          <FormsHeaderText text={"Examination and transvaginal scan"} />
