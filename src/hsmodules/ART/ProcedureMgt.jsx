...
export default function ProcedureMgt() {
  ...
  const handleDelete = () => {
    showActionLoader();
    OrderServ.remove(procedureToDelete._id)
      .then(() => {
        hideActionLoader();
        toast.success(`Procedure request Deleted succesfully`);
        setConfirmationDialog(false);
      })
      .catch((err) => {
        hideActionLoader();
        toast.error("Error deleting procedure request" + err);
      });
  };
    ...
    <>
      ...
      <div className="level">
        ...
        >
          ...
          >
            <CustomTable
              title={"Procedure Management List"}
              columns={ProcedureMgtSchema}
              data={facilities}
              pointerOnHover
              highlightOnHover
              striped
      ...
      <CustomConfirmationDialog
        ...
        confirmationAction={() => handleDelete(procedureToDelete)}
        cancelAction={closeConfirmationDialog}
        type="danger"
        message={`You are about to delete a procedure request: ${
          procedureToDelete?.name
        } created on ${dayjs(procedureToDelete?.createdAt).format("DD-MM-YYYY")}`}
      />
    </>
  );
}
