...
export default function CaseManagementDetails({ handleGoBack }) {
  //const toast = useToast();
  const { showActionLoader, hideActionLoader, state } =
    useContext(ObjectContext);
  const caseMgtServ = client.service('casemgt');
  const caseMgtDatas = state.ProviderRelationshipModule.selectedCaseMgt || {};
  // console.log(caseMgtDatas, 'caseMgtData');

  const { register, control, handleSubmit, watch } = useForm({
    defaultValues: {
      parastatals: caseMgtDatas.hmoname || '',
      address: caseMgtDatas.address || '',
      region: caseMgtDatas.region || '',
      staffName: caseMgtDatas.hmostaffname || '',
      stageStatus: caseMgtDatas.status || '',
      steps:
        caseMgtDatas.process?.map((step) => ({
          id: step.stepId,
          stepname: step.stepname,
          dateDone: step.dateDone || '',
          status: step.status || 'Not Done',
          comment: step.comments || '',
          uploadedUrl: step?.files?.uploadUrl || '',
          subtasks:
  ...
  //console.log(caseMgtDatas, 'caseMgtDatas');
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const updatedData = {
        ...caseMgtDatas,
        hmoname: data.parastatals || caseMgtDatas.hmoname,
        region: data.region || caseMgtDatas.region,
        hmostaffname: data.staffName || caseMgtDatas.hmostaffname,
        status: data.stageStatus || caseMgtDatas.status,
        process: data.steps.map((step, index) => ({
          ...(caseMgtDatas.process[index] || {}),
          stepId: step.id || caseMgtDatas.process[index]?.stepId,
          stepname: step.stepname || caseMgtDatas.process[index]?.stepname,
          dateDone: step.dateDone || caseMgtDatas.process[index]?.dateDone,
          status: step.status || caseMgtDatas.process[index]?.status,
          comments: step.comment || caseMgtDatas.process[index]?.comments,
          subtasks:
            step.subtasks?.map((subtask, subIndex) => ({
              ...(caseMgtDatas.process[index]?.subtasks[subIndex] || {}),
              subtaskName:
                subtask.subtaskName ||
                caseMgtDatas.process[index]?.subtasks[subIndex]?.subtaskName,
              dateDone:
                subtask.dateDone ||
                caseMgtDatas.process[index]?.subtasks[subIndex]?.dateDone,
              status:
                subtask.status ||
                caseMgtDatas.process[index]?.subtasks[subIndex]?.status,
              comments:
                subtask.comment ||
                caseMgtDatas.process[index]?.subtasks[subIndex]?.comments,
              uploadedFiles:
                subtask.uploadedFiles ||
                caseMgtDatas.process[index]?.subtasks[subIndex]?.uploadedFiles,
            })) || [],
        })),
      };

      await caseMgtServ.patch(caseMgtDatas?._id, updatedData);
  ...
  return (
    ...
    >
      <Box mb={2}>
        ...
        <Box mt={4}>
          <FormsHeaderText text=" Case Audit Management Details" />
        </Box>
        <Grid container spacing={2} my={3}>
          ...
          <Grid item xs={4}>
            <CustomSelect
              ...
              options={caseMgtDatas?.process?.flatMap((step) => [
                step.stepname,
      ...
      <Box>
        ...
        {caseMgtDatas?.process?.map((step, stepIndex) => (
          <Accordion
            key={stepIndex}
            elevation={0}
            sx={{
              ...
              >
                ...
                {step.stepId === 5 && (
                  <GlobalCustomButton
                    text="View Report"
                    onClick={() => {
                      if (step?.files?.uploadUrl) {
                        window.open(`${step?.files?.uploadUrl}`, '_blank');
                      } else {
                        toast.error('No report uploaded');
                      }
                    }}
                    customStyles={{
                      marginRight: '5px',
