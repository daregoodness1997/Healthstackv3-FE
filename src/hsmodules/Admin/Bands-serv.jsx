...
/* import dataD from "./Patients_corrected.json" ; */


pdfjs.GlobalWorkerOptions.workerSrc = `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjs.version}/pdf.worker.js`;

/* const dataH = require("../../data/hci/NHIA-OTHERS.json");
const dataD = require("./Patients_corrected.json");
...
export function BandList({ showCreateModal }) {
  ...
  const handleFileUpload3 = async (event) => {
    ...
    for (const unique of uniquePolicy) {
      ...
      let policy = {
        ...
        providers: provi, //
        sponsorshipType:
          faci.CustomerName === "Individual" ? "Self" : "Company",
        sponsor: { facilityName: faci.CustomerName, code: faci.CustomerID },
        plan: {
        ...
        case 0:
          ...
          await facilityServ
            ...
            .then(async (resp) => {
              ...
              await InvoiceServ.create(invoice)
                .then((res) => {
                  console.log("res", res);
          ...
          console.log(
            `0:${zero.length},1:${one.length}, 2:${two.length}, 3:${others.length}`
          );
  ...
  const updateNHisTariff = () => {
    let tariffList = [];
    for (let p = 0; p <= dataH.length - 1; p++) {
      //dataH.length-1
      let service = dataH[p];
        ...
        plans: [
          {
            ...
            capitation: false,
            coPay: true,
            copayDetail: service["% co pay"]
              ? service["% co pay"].toString()
              : "0", //%
            reqPA: true,
            covered: true,
            // comments:{ type: String },
 ...
 let contract=[...oldcontract,...newcontract] */
        ...
        TariffServ.patch(tariff._id, { contracts: tariffList })
          .then((res) => {
            console.log("done updating tariff");
          })
          .catch((err) => {
            console.log(
              "Error updating tariff, probable network issues or " + err
            );
          });
      })
      .catch((err) => {
  ...
  const extractdrugs = async (content) => {
    let serviceList = [];
    let tariffList = [];
    for (let p = 0; p <= content.length - 1; p++) {
      //content.length-1
           ...
           if (items[t+i].str==="NHIS"){
            ...
            for (let q = i + 1; q <= items.length - 1; q++) {
              ...
              if (nextI === "NHIS") {
                ...
                if (
                  !result &&
                  service.cost.length < 9 &&
                  (service.cost !== "0.15g" ||
                    service.cost !==
                      "18.2.2 VACCINES FOR SPECIFIC IMMUNIZATION" ||
                    service.cost !== "13% for dilution")
                ) {
    ...
    TariffServ.find({
      ...
      .then((res) => {
        ...
        TariffServ.patch(tariff._id, { contracts: contract })
          .then((res) => {
            console.log("done updating tariff");
          })
          .catch((err) => {
 ...
 */
  ...
  const analyse = async (pageText) => {
    pageText.items.forEach(async (item, i) => {
      ...
      if (
        item.str === "Issued by NHIS" ||
        item.str === "HEALTHCARE INTERNATIONAL LIMITED" ||
        item.str === "" ||
        item.str === " " ||
        item.str === "8/1/2023" ||
        item.str === "page"
  ...
  const createfacility = async (facilityinfo) => {
    ...
    if (facility.total > 0) {
      ...
      console.log(
        "creating new provider for :" + facilityinfo.code,
        facilityinfo.name
      );
      ...
      await facilityServ
        ...
        .catch((err) => {
          console.log(
            "facility rellationship not created :" + err,
            facilityDocument
          );
        });
    }
  ...
  const createsponsor = async () => {
    let facilitydata = {
      ...
      facilityAddress:
        " Plot 297, P.O.W. Mafemi Crescent, Off Solomon Lar Way, Utako District, Abuja.",
  ...
  const createpolicy = async (provList) => {
    ...
    for (let i = 0; i <= provList.length - 1; i++) {
      ...
      for (let p = 0; p <= facilityinfo.patients.length - 1; p++) {
        ...
        policy.beneficiaries = [principal1, ...policy.dependents];
...
const updateClientList = async ()=>{
  ...
  alert("working")
    ...
    console.log(defaultEmail)
      ...
      try {
       const client= await ClientServ.create(clientData);
       console.log(client)
      } catch (err) {
  ...
  return (
    <>
      ...
      <input type="number" value={end} name="end" onChange={(e)=> setEnd(e.target.value) } /> */}
        <GlobalCustomButton onClick={updateClientList}>
          Update Patient List
