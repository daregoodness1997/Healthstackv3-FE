...
export function TransferDetail({ openModifyModal }) {
  ...
  // generate printable HTML for a transfer entry
  const generateTransferHtml = (entry) => {
    ...
    const rows = (entry.productitems || [])
      .map((it, i) => {
        ...
        const amount = it.amount != null ? Number(it.amount).toLocaleString() : '';
        return `
          <tr>
    ...
    return `<!doctype html>
      <html>
        <head>
          <meta charset="utf-8" />
          <title>Transfer - ${entry.documentNo || ''}</title>
          <style>
        ...
        <body>
          ...
          <div style="margin-top:12px" class="right"><strong>Total Amount:</strong> ${entry.org_totalamount ?? ''}</div>
        </body>
      </html>`;
  };
  ...
  const handlePrint = () => {
    if (!ProductEntry) {
      toast.info('No transfer selected to print');
      return;
    }
    const html = generateTransferHtml(ProductEntry);
...
export function RecvTransferDetail({ handleCloseRecvDetailModal }) {
  ...
  // generate printable HTML for received transfer
  const generateTransferHtml = (entry) => {
    if (!entry) return '<!doctype html><html><body><p>No data to print</p></body></html>';
    ...
    const orgDate = entry.org_date ? dayjs(entry.org_date).format('DD-MM-YY HH:mm') : '';
    const totalSent = entry.productitems?.reduce((acc, item) => acc + item.amount, 0) || 0;

    return `<!doctype html>
      <html>
        <head>
          <meta charset="utf-8" />
          <title>Received Transfer - ${entry.documentNo || ''}</title>
          <style>
            body{font-family:Arial,Helvetica,sans-serif;margin:16px;color:#222}
            h2{margin:0 0 8px 0}
            table{border-collapse:collapse;width:100%;margin-top:8px}
        ...
        <body>
          <h2>Received Transfer ${entry.documentNo || ''}</h2>
          <div class="meta">
            <strong>From:</strong> ${entry.org_store_name || ''} &nbsp;
            <strong>To:</strong> ${entry.dest_store_name || ''} &nbsp;
  ...
  const submitTransfer = async () => {
    try {
      ...
      let transferEntry = {
        ...
        source: state.StoreModule.selectedStore.name,
        sourceId: state.StoreModule.selectedStore._id,
        sourceType: 'Store',
        date: new Date(),
