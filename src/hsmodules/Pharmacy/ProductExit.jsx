...
export function ProductExitList({ openDetailModal, openCreateModal }) {
  ...
  // Convert browser/local date string to Date object
  const localDate = new Date(dateString);

  // Start of local day (midnight in user's timezone)
  ...
  // End of local day (next day's midnight)
  ...
  // Convert local boundaries to UTC-based ISO strings
  ...
  let  query= {
      ...
      if (startDate) {
                console.log(startDate)
                const dayRange = buildDayRange(startDate);
  ...
  const ProductExitSchema = [
    ...
    },
    {
      ...
      selector: (row) => dayjs(row.date).format("DD-MM-YY HH:mm"),
            ...
            <TableMenu>
              <div style={{ display: "flex", alignItems: "center" }}>
                ...
                <h2 style={{ marginLeft: "10px", fontSize: "0.95rem" }}>
                 Issued Out List (Dispense /Transfer out)
                </h2>
                  <MuiClearDatePicker
                    value={startDate}
