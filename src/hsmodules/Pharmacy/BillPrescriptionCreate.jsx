...
export default function BillPrescriptionCreate({ closeModal }) {
  ...
  const getSearchfacility = async (obj1) => {
    ...
    setObj(obj1);
    setObjService(obj.billingDetails);
  };
  useEffect(() => {}, [obj]);
...
export function InventorySearch({ getSearchfacility, clear, label }) {
  ...
  const findmainstores = async()=>{
    const list = await locationServ.find({
      query:{
      ...
      productServ
        ...
        .then((res) => {
          //  console.log("product  fetched successfully")
            ...
            console.log(state.StoreModule.selectedStore)
