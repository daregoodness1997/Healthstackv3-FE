    ...
    <>
      <Box sx={{ width: "600px" }}>
        <Grid container spacing={1} pt={1} mb={2}>
          ...
          <Grid item xs={12}>
            <Input
              label="Product SubClassification"
              name="subclassification"
              type="text"
              register={register("subclassification",)}
            />
          </Grid>
          <Grid item xs={12}>
            <Input
...
export function ProductModify({ product, closeModal }) {
  ...
  // Use product prop if provided, otherwise fall back to state
  const Product = product || state.ProductModule.selectedProduct;
  ...
  const onSubmit = (data, e) => {
    ...
    ProductServ.patch(Product._id, data)
      .then(res => {
        ...
        // Call closeModal if provided (for modal usage), otherwise change state
        if (closeModal) {
          closeModal();
        } else {
          changeState();
    ...
    <>
      <Box sx={{ width: "600px" }}>
        ...
        <Box sx={{ display: "flex" }}>
          ...
          <GlobalCustomButton
            onClick={closeModal || handleCancel}
            variant="outlined"
            color="error"
          >
            Cancel
