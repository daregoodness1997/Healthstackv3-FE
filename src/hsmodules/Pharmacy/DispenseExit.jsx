...
export function ProductExitCreate({closeModal}) {
  ...
  useEffect(() => {
    console.log("state", state.financeModule)
    if (!!medication) {
      const oldname =
      ...
      const updatedDispenseProducts = dispenseProducts.map(item => {
        ...
        newItem.serviceInfo.instruction=newItem.orderInfo.orderObj.instruction
        newItem.serviceInfo.disabled=true
  ...
  newRow.amount= row.quantity*row.price
  setProductItem(prev=>{
    const index=prev.findIndex(item=>item.billingId===row.billingId)
    ...
    },
    {
      ...
      selector: row =>(
        ...
        <input
          ...
          type="number"
          value={row.quantity}    //{itemStates[row._id].quantity}
