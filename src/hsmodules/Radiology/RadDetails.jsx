  ...
  const onSubmit = async (data, e) => {
    ...
    if (bill_report_status === "Pending") {
      ClientServ.create(document)
        .then((res) => {
          setSuccess(true);
          toast.success("Radiology Report created succesfully");
          setSuccess(false);
        })
        .catch((err) => {
          toast.error("Error creating Radiology Report " + err);
        });
    }
    ...
    if (bill_report_status === "Draft") {
      ClientServ.patch(order.resultDetail._id, document)
        .then((res) => {
          setSuccess(true);
          toast.success("Radiology Report updated succesfully");
          setSuccess(false);
        })
        .catch((err) => {
          toast.error("Error updating Radiology Report " + err);
        });
    }
      ...
      <Grid container spacing={2} sx={{ alignItems: "center" }}>
        <Grid item md={6}>
          <p style={{ maxWidth: "400px", fontWeight: "700" }}>
            {ProperCase(
              `Radiology Report for ${order.orderInfo.orderObj.clientname} Ordered Test: ${order.serviceInfo.name}`
            )}
          </p>
        </Grid>
        <Grid item md={6} mb={1}>
