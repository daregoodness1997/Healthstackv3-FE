...
export default function NhaStatutoryDetails({ handleGoBack }) {
  ...
  const NhiaStatutoryData =
    state.ProviderRelationshipModule.selectedNhiaStatutory || [];
  console.log(NhiaStatutoryData);

  const { control, handleSubmit, watch, register } = useForm({
    defaultValues: {
      providerName: NhiaStatutoryData.hmoname,
      address: NhiaStatutoryData.address,
      region: NhiaStatutoryData.region,
      staffName: NhiaStatutoryData.hmostaffname,
      stageStatus: NhiaStatutoryData.status,
      ...NhiaStatutoryData.process.reduce((acc, stage) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const updatedData = {
        ...NhiaStatutoryData,
        facilityname: data.providerName || NhiaStatutoryData.hmoname,
        address: data.address || NhiaStatutoryData.address,
        region: data.region || NhiaStatutoryData.region,
        hmostaffname: data.staffName || NhiaStatutoryData.hmostaffname,
        status: data.stageStatus || NhiaStatutoryData.status,
        process: NhiaStatutoryData.process.map((stage) => ({
          ...stage,
      ...
      const response = await NhiaStatutoryServ.patch(
        NhiaStatutoryData._id,
        updatedData,
      );
  ...
  const stageStatuses = watch(
    NhiaStatutoryData.process.map((stage) => `stageStatus${stage.stepId}`),
  );
    ...
    >
      <>
          ...
          <Grid container spacing={2} my={3}>
            ...
            <Grid item xs={4}>
              <CustomSelect
                ...
                options={NhiaStatutoryData?.process?.reduce((acc, step) => {
        ...
        <Box>
          ...
          {NhiaStatutoryData.process.map((stage, index) => (
            <Accordion
              key={stage.stepId}
