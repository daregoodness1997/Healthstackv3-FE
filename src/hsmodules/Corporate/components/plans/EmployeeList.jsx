...
export function ListEmployee({selectedemployee, limit, showLink}) {
  ...
  const getFacilities = async () => {
    ...
    let prevChosen=[]
      ...
      const findAllchosenEmployees= await linkServer.find({
        query:{
          
          invoiceId:state.InvoiceModule.selectedInvoice._id ,
          
        }
       })
       findAllchosenEmployees.data.forEach(el => {
        prevChosen.push(el.employee)
        
        
      });
     
      //console.log("facilities", facilities);
   const findchosenEmployees= await linkServer.find({
    query:{
   ...
   setLinkedEmployees(findchosenEmployees.data)

    findchosenEmployees.data.forEach(el => {
      el.employee.used=el.used
      emp.push(el.employee)
    ...
    const idsToRemove = new Set(prevChosen.map(obj => obj._id));
    console.log(idsToRemove)

    const filteredArray = findEmployee.data.filter(obj => !idsToRemove.has(obj._id));
    console.log(filteredArray)
    setFacilities(filteredArray)
    setLoading(false);
  };
  ...
  const handleChoseClient = (e, row) => {
   ...
   const removeEmployee=async (row)=>{
    const  empRemove= linkedEmployees.find(el=>el.employeeId===row._id)
    console.log("employee2remove", empRemove)
    if (!row.used){
    await linkServer.remove(empRemove._id)
      .then(res=>{
        setPlans(prev=>prev.filter(el=> el._id!= row._id))
        toast.success("Employee successfully removed from plan")
        getFacilities()
      })
      .catch(err=>{
        console.log(err)
        toast.error("Error removing employee from plan " + err)
   })
  }else{
    toast.success("Employee has already registered with plan and cannot be removed from plan")
  }
  ...
  const getEmployeeSchema2 = [
   ...
   /*  {
    ...
    }, */
    {
      ...
      key: "department",
      description: "Enter department",
      selector: row => row.used? row.used.toString():"Not Yet",
      sortable: true,
      required: true,
      inputType: "TEXT",
      validator: yup.string().required("Enter your Department"),
         ...
         { plans.length>0 &&  <div
            ...
            >
              <CustomTable
                ...
                highlightOnHover
                striped
               onRowClicked={(row)=>handleShowLink(row)}
                progressPending={loading}
              />
            </div>}
           </div>
