...
// console.log(state.PolicyModule,"POLICY-MODE")
  ...
  const getHealthPlans = useCallback(async () => {
    ...
    const orgId = isHMO
      ? facility._id
      : state.PolicyModule.selectedPolicy.organization._id;
          ...
          <>
            <Box p={2}>
              <Grid container spacing={2}>
                ...
                <Grid item md={isHMO ? 4 : 4}>
                  <Input
                    value={state?.PolicyModule?.preservedPolicy?.plan?.premium}
                    disabled
            ...
            <Box p={2}>
              ...
              <CustomTable
                ...
                data={
                  policy?.providers?.[0]?.[0] ? [policy.providers[0][0]] : []
                }
                pointerOnHover
                highlightOnHover
                striped
                onRowClicked={(row) => onFacilityRowClicked(row)}
                progressPending={false}
