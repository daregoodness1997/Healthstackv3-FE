...
import { formatDistanceToNowStrict, format, subDays, addDays, startOfDay } from 'date-fns'
...
const AppointmentList = ({ showDetail, showCreate, module }) => {
    ...
    const handleRow = async appointmentId => {
        const getAppointmentById = await appointmentsServer.get(
            appointmentId?._id,
        )
        ...
        setState(prev => ({
            ...
            AppointmentModule: {
                ...
                // selectedAllAppointment: appointment,
            },
        }))
    ...
    const handleSearch = val => {
        ...
        appointmentsServer
            ...
            .catch(err => {
                return err
                toast.error(
                    'Error Searching Client, probable network issues ' + err,
                )
            })
    }
    ...
    }, [state.employeeLocation, startDate,page, limit])
    ...
    const mapFacilities = () => {
        ...
        appointments.map((facility, i) => {
            mapped.push({
                ...
                startDate: dayjs(facility.start_time).format(
                    'DD/MM/YYYY, hh:mm',
                ),
                id: i,
