...
const AppointmentDetail = ({closeModal, module}) => {
  ...
  useEffect(() => {
    ...
    const data = {
      ...
      age: formatDistanceToNowStrict(dayjs(appointment.dob).toDate()),
  ...
  const checkinPatient = (data, e) => {
    showActionLoader();
    appointmentsServer
      .patch(appointment._id, {checkedin_at: new Date(), appointment_status: "Checked In"})
      .then(res => {
        hideActionLoader();
  ...
  const checkOutPatient = () => {
    showActionLoader();
    appointmentsServer
      .patch(appointment._id, {checkedout_at: new Date(), appointment_status: "Checked Out"})
      .then(res => {
        hideActionLoader();
  ...
  const hanldeAttendToClient = async () => {
   /*  if (appointment?.client?.paymentinfo?.some(checkHMO)) {
    ...
    if (appointment.client) {
     ...
     localStorage.setItem("client",JSON.stringify(appointment.client) )
      ...
      await appointmentsServer.patch(appointment._id, {encounter_start_time: new Date()});
      localStorage.setItem("client",JSON.stringify(patient) )
      navigate("/app/clinic/documentation");
    }
  };
